...
with open('regeneratedresults.csv', 'wb') as outfile:
    ...
    brow.append("MATCHED ENTITIES")
    brow.append("OVERMATCHED ENTITIES")
    ...
    while(i<len(queries)):
        ...
        brow.append(RESULTENTITIES[i])
        brow.append(ENTITIES[i])
        brow.append(str(Matchedentities[i])+" in "+str(Totalentities[i]))
        brow.append(str(Overmatchedentities[i])+" in "+str(Totalentities[i]))
        writer.writerow(brow)
        i+=1
